...
is_review_active() {
  ...
  review_requests="$(gh pr view "$pr_number" --json reviewRequests --jq '.reviewRequests[].login' 2>/dev/null || true)"
  if printf '%s\n' "$review_requests" | grep -Fxq "$reviewer"; then
    return 0
  fi

  reviews="$(gh pr view "$pr_number" --json reviews --jq '.reviews[].author.login' 2>/dev/null || true)"
