...
#!/bin/sh
env OCP_NO_VERSION_CHECK=1 OCP_CONFIG_DIR=build/vhs-demo/config OCP_CACHE_DIR=build/vhs-demo/cache OCP_OPENCODE_DIR=build/vhs-demo/opencode OCP_WORKING_DIR=build/vhs-demo/workspace TERM=xterm-256color COLORTERM=truecolor CLICOLOR_FORCE=1 sh -c 'if script -q -c true /dev/null >/dev/null 2>&1; then exec script -q -c "./build/install/ocp/bin/ocp" /dev/null; else exec script -q /dev/null ./build/install/ocp/bin/ocp; fi'
SH
